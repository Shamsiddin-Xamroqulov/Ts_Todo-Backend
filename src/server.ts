...
const server = http.createServer(async (req, res) => {
    ...
    if(reqUrl.startsWith("/api")){
        ...
        if(await checkToken(req, res)){
            if(reqUrl.startsWith("/api/todo/") && reqMethod == METHODS_ENUM.DELETE) userTodos.deleteTodo(req, res);
            ...
            if(reqUrl.startsWith("/api/update/") && reqMethod == METHODS_ENUM.UPDATE) userTodos.UpdateTodo(req, res);
        }
